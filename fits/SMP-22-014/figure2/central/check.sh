...
grep -Eq 'FROM MIGRAD +STATUS=CONVERGED' output/minuit.out.txt
grep 'FROM HESSE' output/minuit.out.txt | tail -n 1 | grep -Eq 'STATUS=OK'
...
echo "OK: converged central fit, accurate final HESSE, and 16 symmetric PDF error members."
